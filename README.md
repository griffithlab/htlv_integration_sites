...
Using just the mark duplicate results
```bash
isub -m 32
source $WORKING_DIR/git/htlv_integration_sites/envs.txt
...
for FASTQ_NAME in "${FASTQ_NAMES[@]}"; do
    SAMPLE=$(echo $FASTQ_NAME | awk -F_ '{print $2}')
    awk -v sample="$SAMPLE" '{print $0 "\t" sample}' counts/${SAMPLE}.markedsorted_filtered_merged.bed.tsv > tmp/${SAMPLE}.markedsorted_filtered_merged.bed.tsv
done
echo -e "chromosome\tstart_pos\tend_pos\tcount\tsample" > tmp/header.tsv
cat tmp/header.tsv tmp/*markedsorted_filtered_merged.bed.tsv > counts/ALL.markedsorted_filtered_merged.bed.tsv

exit
```


Using reads with hits to viral genome
```bash
isub -m 32
source $WORKING_DIR/git/htlv_integration_sites/envs.txt
